...
const Analytics = () => {
  ...
  // dex total liquidity chart data
  const feesData: Record<ChartViewType, { xData: string[] | number[]; yData: number[] }> = {
    "24H": {
      xData: [
        1738350000, 1738353600, 1738357200, 1738360800, 1738364400, 1738368000, 1738371600, 1738375200,
        1738378800, 1738382400, 1738386000, 1738389600, 1738393200, 1738396800, 1738400400, 1738404000,
      ...
      yData: [
        159363.65, 484070.06, 21611.37, 116807.54, 332380.60, 20576.92, 151315.31, 2832.97,
        182333.19, 189270.81, 20011.36, 11068.69, 63289.52, 60408.43, 7965.73, 14911.46,
        114244.91, 212934.89, 43974.66, 225807.61, 79782.11, 342533.87, 125326.12, 163123.09
      ],
    },
    "1M": {
      xData: [
  ...
  const [totalFeesView, setTotalFeesView] = useState<ChartViewType>('24H');
    ...
    <div className="analytics-page-height mb-5">
      ...
      <Row className='mt-4'>
        <Col xs={12} lg={12} className='mt-3 mb-5'>
          <FeesChart
            xData={feesData[totalFeesView].xData}
            yData={feesData[totalFeesView].yData}
            view={totalFeesView}
            setView={setTotalFeesView}
          />
        </Col>
      </Row>
    </div>
