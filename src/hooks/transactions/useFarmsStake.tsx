...
export const useFarmsStake = (token_id: string, token_decimals: number, token_amount: number) => {
  ...
  const stake = async () => {
    ...
    const sessionId = await sendAndSignTransactionsWrapped(
      [transaction],
      transactionDisplayInfo({ transactionName: 'stake ', successTransactionName: 'Staked' })
    );
    return sessionId;
  };
